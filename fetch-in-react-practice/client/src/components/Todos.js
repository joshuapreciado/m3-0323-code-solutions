...
export default function Todos() {
  ...
  async function addTodo(newTodo) {
    fetch(`/api/todos`, {
      ...
      setTodos((donetodos) => donetodos.concat(userData))
    }).catch(error => {
      setError(error)
    })
  }
  ...
  async function toggleCompleted(todoId) {
    let todoID = todos.findIndex(list => {
      return list.todoId === todoId;
    })
    const test = todos[todoID].isCompleted;
    const opposite = {"isCompleted": !test};
